...
// at the end, and the resulting formula — click any "Retorno" number in the
...
function formatPct(v, digits = 2) {
  return v === null || v === undefined || Number.isNaN(v) ? "n/d" : `${(v * 100).toFixed(digits)}%`;
}
...
function ComponentDetail({ c }) {
  ...
  return (
    ...
    >
      ...
      {missing ? (
        <p style={{ ...ui_muted, margin: 0 }}>Sin datos suficientes en esta ventana para este activo.</p>
      ) : (
        <>
          <table style={{ width: "100%", fontSize: 12.5, borderCollapse: "collapse" }}>
            <tbody>
              <tr>
                <td style={cellLabelStyle}>Ventana solicitada</td>
                <td style={cellValueStyle}>
                  {formatDate(c.windowStart)} → {formatDate(c.windowEnd)}
                </td>
              </tr>
              <tr>
                <td style={cellLabelStyle}>Fecha / precio inicial</td>
                <td style={cellValueStyle}>
                  {formatDate(c.startDate)} · <strong>{formatPrice(c.startPrice)}</strong>
                </td>
              </tr>
              <tr>
                <td style={cellLabelStyle}>Fecha / precio final</td>
                <td style={cellValueStyle}>
